...
export function WeatherWidgets({ location }: WeatherWidgetsProps) {
    ...
    return (
        <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide">
            {/* Temperature Widget */}
            <div className="flex-shrink-0 bg-black/80 backdrop-blur-xl rounded-xl p-4 min-w-[100px] border border-white/10">
                <div className="text-3xl mb-1">{weather.weatherIcon}</div>
                <div className="text-2xl font-bold text-white">{weather.temperature}°</div>
                <div className="text-xs text-white/60 mt-1 truncate">{weather.weatherCondition}</div>
            </div>
            ...
            <div className="flex-shrink-0 bg-black/80 backdrop-blur-xl rounded-xl p-4 min-w-[100px] border border-white/10">
                ...
                <div className="text-xs text-white/60 mt-1">Asfalt</div>
            </div>
            ...
            <div className="flex-shrink-0 bg-black/80 backdrop-blur-xl rounded-xl p-4 min-w-[100px] border border-white/10">
                ...
                <div className="text-xs text-white/60 mt-1">Cabrio İndeks</div>
            </div>

            {/* Wind Widget */}
            <div className="flex-shrink-0 bg-black/80 backdrop-blur-xl rounded-xl p-4 min-w-[100px] border border-white/10">
                <div className="text-2xl mb-1">💨</div>
                <div className="text-lg font-bold text-white">{weather.windSpeed} <span className="text-sm text-white/60">km/s</span></div>
                <div className="text-xs text-white/60 mt-1">Rüzgar</div>
            </div>

            {/* Humidity Widget */}
            <div className="flex-shrink-0 bg-black/80 backdrop-blur-xl rounded-xl p-4 min-w-[100px] border border-white/10">
                <div className="text-2xl mb-1">💧</div>
                <div className="text-lg font-bold text-white">{weather.humidity}<span className="text-sm text-white/60">%</span></div>
                <div className="text-xs text-white/60 mt-1">Nem</div>
            </div>

            {/* Rain Warning (if applicable) */}
            {weather.isRaining && (
                <div className="flex-shrink-0 bg-gradient-to-br from-blue-600/30 to-blue-800/30 backdrop-blur-xl rounded-xl p-4 min-w-[100px] border border-blue-500/50">
                    <div className="text-2xl mb-1">🌧️</div>
